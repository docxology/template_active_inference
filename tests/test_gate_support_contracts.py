"""Contract checks for gate artifact bootstrap helpers."""

from __future__ import annotations

import importlib.util
import os
from dataclasses import replace
from pathlib import Path
from types import SimpleNamespace

import gate_support
import pytest


def _runtime(**overrides: object) -> gate_support.GateArtifactRuntime:
    runtime = gate_support.GateArtifactRuntime(
        bootstrapped_roots=set(),
        bootstrapped_signatures={},
    )
    return replace(runtime, **overrides)


def _load_tests_conftest():
    conftest_path = Path(__file__).with_name("conftest.py")
    spec = importlib.util.spec_from_file_location("active_inference_tests_conftest", conftest_path)
    assert spec is not None
    assert spec.loader is not None
    project_conftest = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(project_conftest)
    return project_conftest


def test_required_gate_artifact_signature_uses_content_hash(
    tmp_path: Path,
) -> None:
    rel = "output/data/cache_probe.json"
    path = tmp_path / rel
    path.parent.mkdir(parents=True)
    fixed_time_ns = 1_700_000_000_000_000_000

    runtime = _runtime(required_artifacts=(rel,))
    path.write_text("alpha", encoding="utf-8")
    os.utime(path, ns=(fixed_time_ns, fixed_time_ns))
    first = gate_support._required_gate_artifacts_signature(tmp_path, runtime=runtime)

    path.write_text("bravo", encoding="utf-8")
    os.utime(path, ns=(fixed_time_ns, fixed_time_ns))
    second = gate_support._required_gate_artifacts_signature(tmp_path, runtime=runtime)

    assert first is not None
    assert second is not None
    assert second != first


def test_session_prewarm_skips_collect_only() -> None:
    project_conftest = _load_tests_conftest()

    def fail_if_called() -> None:
        raise AssertionError("collect-only discovery must not prewarm gate artifacts")

    session = SimpleNamespace(config=SimpleNamespace(option=SimpleNamespace(collectonly=True)))

    project_conftest._prewarm_gate_artifacts(session, source_iterator=fail_if_called)


def test_collection_hook_skips_collect_only_before_selection() -> None:
    project_conftest = _load_tests_conftest()
    session = SimpleNamespace(config=SimpleNamespace(option=SimpleNamespace(collectonly=True)))
    config = SimpleNamespace(option=SimpleNamespace(collectonly=True))
    items = [SimpleNamespace(path=Path("tests/gates/test_manuscript_gates.py"))]

    project_conftest.pytest_collection_modifyitems(session, config, items)


def test_direct_only_selection_skips_gate_prewarm() -> None:
    project_conftest = _load_tests_conftest()
    direct_items = [SimpleNamespace(path=Path("tests/test_fixed_point_direct.py"))]
    mixed_items = [*direct_items, SimpleNamespace(path=Path("tests/gates/test_manuscript_gates.py"))]

    assert project_conftest._selection_needs_gate_prewarm(direct_items) is False
    assert project_conftest._selection_needs_gate_prewarm(mixed_items) is True


def test_mutable_output_snapshot_includes_gate_contract_artifacts() -> None:
    project_conftest = _load_tests_conftest()

    rels = {
        path.relative_to(project_conftest.PROJECT_ROOT).as_posix()
        for path in project_conftest._iter_mutable_project_outputs()
    }

    assert "output/data/artifact_provenance.json" in rels
    assert "output/data/sheaf_gluing_certificate.json" in rels
    assert "output/reports/artifact_diffoscope.json" in rels


def test_ensure_gate_artifacts_reuses_matching_session_signature(
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()

    def fail_if_called(project_root: Path) -> bool:
        raise AssertionError("_gate_artifacts_present should not run for cached signatures")

    runtime = _runtime(
        bootstrapped_signatures={root: "prepared"},
        signature_override=lambda project_root: "prepared",
        artifacts_present_override=fail_if_called,
    )

    gate_support.ensure_gate_artifacts(root, runtime=runtime)

    assert root in runtime.bootstrapped_roots


def test_ensure_gate_artifacts_records_complete_bootstrapped_changed_signature(
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()
    validated_roots: list[Path] = []

    def artifacts_are_valid(project_root: Path) -> bool:
        validated_roots.append(project_root)
        return True

    def fail_if_called(*args, **kwargs) -> None:
        raise AssertionError("complete bootstrapped artifact trees should not trigger a fixed-point refresh")

    runtime = _runtime(
        bootstrapped_roots={root},
        bootstrapped_signatures={root: "old"},
        signature_override=lambda project_root: "changed",
        artifacts_present_override=artifacts_are_valid,
        analysis_runner=fail_if_called,
    )

    gate_support.ensure_gate_artifacts(root, runtime=runtime)

    assert runtime.bootstrapped_signatures[root] == "changed"
    assert validated_roots == [root]


def test_ensure_gate_artifacts_rejects_invalid_bootstrapped_changed_signature(
    monkeypatch,
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()
    monkeypatch.setenv(gate_support._ALLOW_GATE_REBUILD_ENV, "1")

    def stop_before_rebuild(*args, **kwargs) -> None:
        raise RuntimeError("full refresh required")

    runtime = _runtime(
        bootstrapped_roots={root},
        bootstrapped_signatures={root: "old"},
        signature_override=lambda project_root: "changed",
        required_exist_override=lambda project_root: True,
        artifacts_present_override=lambda project_root: False,
        analysis_runner=stop_before_rebuild,
    )

    with pytest.raises(RuntimeError, match="full refresh required"):
        gate_support.ensure_gate_artifacts(root, runtime=runtime)

    assert runtime.bootstrapped_signatures[root] == "old"


def test_ensure_gate_artifacts_fails_fast_without_rebuild_opt_in(monkeypatch, tmp_path: Path) -> None:
    root = tmp_path.resolve()
    monkeypatch.delenv(gate_support._ALLOW_GATE_REBUILD_ENV, raising=False)

    def fail_if_called(*args, **kwargs) -> None:
        raise AssertionError("standard pytest must not rebuild active-inference artifacts")

    runtime = _runtime(
        signature_override=lambda project_root: "stale",
        artifacts_present_override=lambda project_root: False,
        readiness_override=lambda project_root: ("semantic drift",),
        analysis_runner=fail_if_called,
    )

    with pytest.raises(AssertionError, match="Standard pytest runs do not rebuild"):
        gate_support.ensure_gate_artifacts(root, runtime=runtime)


def test_refresh_generated_gate_artifacts_accepts_valid_changed_signature(
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()

    def fail_if_called(project_root: Path, out: Path, *, passes: int | None = None) -> None:
        raise AssertionError("valid changed artifact trees should not rebuild")

    runtime = _runtime(
        bootstrapped_signatures={root: "old"},
        signature_override=lambda project_root: "changed",
        artifacts_present_override=lambda project_root: True,
        settle_override=fail_if_called,
    )

    gate_support.refresh_generated_gate_artifacts(root, force=False, runtime=runtime)

    assert runtime.bootstrapped_signatures[root] == "changed"
    assert root in runtime.bootstrapped_roots


def test_refresh_generated_gate_artifacts_rejects_invalid_post_rebuild_signature(
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()
    runtime = _runtime(
        signature_override=lambda project_root: "invalid-after-rebuild",
        settle_override=lambda project_root, out: None,
        artifacts_present_override=lambda project_root: False,
    )

    with pytest.raises(AssertionError, match="gate artifacts are invalid"):
        gate_support.refresh_generated_gate_artifacts(root, runtime=runtime)

    assert root not in runtime.bootstrapped_signatures


def test_settle_generated_contracts_delegates_to_semantic_fixed_point(
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()
    out = root / "output" / "data" / "manuscript_variables.json"
    out.parent.mkdir(parents=True)
    out.write_text("{}", encoding="utf-8")
    calls: list[tuple[Path, bool, int]] = []
    sheaf_prerequisite_calls: list[tuple[Path, bool]] = []

    def write_sheaf_prerequisites(project_root: Path, *, finalize: bool = True) -> dict:
        sheaf_prerequisite_calls.append((project_root, finalize))
        return {}

    def run_fixed_point(project_root: Path, *, require_analysis_outputs: bool, max_passes: int) -> dict:
        calls.append((project_root, require_analysis_outputs, max_passes))
        return {}

    runtime = _runtime(
        figure_writer=lambda project_root: None,
        gif_writer=lambda project_root: None,
        animation_writer=lambda project_root: None,
        sheaf_writer=write_sheaf_prerequisites,
        semantic_fixed_point_runner=run_fixed_point,
    )
    gate_support._settle_generated_contracts(root, out, passes=2, runtime=runtime)

    assert calls == [(root, False, 8), (root, False, 8)]
    assert sheaf_prerequisite_calls == [(root, False), (root, False)]


def test_refresh_gate_artifact_session_signature_records_valid_current_signature(
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()

    def fail_if_called(*args, **kwargs) -> None:
        raise AssertionError("session-signature refresh must not regenerate artifacts")

    runtime = _runtime(
        signature_override=lambda project_root: "fresh",
        artifacts_present_override=lambda project_root: True,
        settle_override=fail_if_called,
    )

    gate_support.refresh_gate_artifact_session_signature(root, runtime=runtime)

    assert runtime.bootstrapped_signatures[root] == "fresh"
    assert root in runtime.bootstrapped_roots


def test_refresh_gate_artifact_session_signature_rejects_invalid_current_signature(
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()
    runtime = _runtime(
        signature_override=lambda project_root: "fresh",
        artifacts_present_override=lambda project_root: False,
    )

    with pytest.raises(AssertionError, match="gate artifacts are invalid"):
        gate_support.refresh_gate_artifact_session_signature(root, runtime=runtime)

    assert root not in runtime.bootstrapped_signatures


def test_refresh_output_gate_contracts_reuses_clean_contracts(tmp_path: Path) -> None:
    root = tmp_path.resolve()

    def fail_if_called(*args, **kwargs) -> None:
        raise AssertionError("clean output gate contracts should not be regenerated")

    runtime = _runtime(
        validate_animation=lambda project_root: [],
        validate_integration=lambda project_root: [],
        validate_sheaf=lambda project_root: [],
        signature_override=lambda project_root: "clean",
        artifacts_present_override=lambda project_root: True,
        animation_writer=fail_if_called,
        integration_writer=fail_if_called,
        sheaf_writer=fail_if_called,
    )

    gate_support.refresh_output_gate_contracts(root, runtime=runtime)

    assert runtime.bootstrapped_signatures[root] == "clean"


def test_refresh_output_gate_contracts_revalidates_repaired_artifacts(tmp_path: Path) -> None:
    root = tmp_path.resolve()
    integration_results = iter([["initial integration issue"], ["still bad"]])
    sheaf_results = iter([["initial sheaf issue"], []])

    runtime = _runtime(
        validate_animation=lambda project_root: [],
        validate_integration=lambda project_root: next(integration_results),
        validate_sheaf=lambda project_root: next(sheaf_results),
        signature_override=lambda project_root: "sig-after-bad",
        integration_writer=lambda project_root: None,
        sheaf_writer=lambda project_root: None,
    )

    with pytest.raises(AssertionError, match="output gate contracts remain invalid"):
        gate_support.refresh_output_gate_contracts(root, runtime=runtime)

    assert root not in runtime.bootstrapped_signatures


def test_gate_artifacts_present_requires_claim_ledger(tmp_path: Path) -> None:
    runtime = _runtime(
        required_artifacts=(),
        validate_animation=lambda project_root: [],
        validate_semantic=lambda project_root: [],
        validate_integration=lambda project_root: [],
        validate_sheaf=lambda project_root: [],
        validate_claims=lambda project_root: False,
    )
    assert gate_support._gate_artifacts_present(tmp_path, runtime=runtime) is False


def test_gate_artifacts_present_requires_animation_deltas(tmp_path: Path) -> None:
    runtime = _runtime(
        required_artifacts=(),
        validate_animation=lambda project_root: ["bad animation"],
        validate_semantic=lambda project_root: [],
        validate_integration=lambda project_root: [],
        validate_sheaf=lambda project_root: [],
        validate_claims=lambda project_root: True,
    )
    assert gate_support._gate_artifacts_present(tmp_path, runtime=runtime) is False


def test_ensure_gate_artifacts_rejects_invalid_post_rebuild_signature(
    monkeypatch,
    tmp_path: Path,
) -> None:
    root = tmp_path.resolve()
    monkeypatch.setenv(gate_support._ALLOW_GATE_REBUILD_ENV, "1")

    def noop(*args, **kwargs) -> None:
        return None

    runtime = _runtime(
        signature_override=lambda project_root: "invalid-after-rebuild",
        artifacts_present_override=lambda project_root: False,
        pymdp_probe=lambda: True,
        analysis_runner=noop,
        simulation_runner=noop,
        policy_comparison_writer=noop,
        policy_grid_writer=noop,
        graph_writer=noop,
        statistics_writer=noop,
        section_composer=noop,
        coverage_writer=noop,
        figure_writer=noop,
        gif_writer=noop,
        animation_writer=noop,
        validation_spine_writer=noop,
        toy_sweep_writer=noop,
        formal_interop_writer=noop,
        sheaf_writer=noop,
        settle_override=noop,
    )

    with pytest.raises(AssertionError, match="gate artifacts are invalid"):
        gate_support.ensure_gate_artifacts(root, runtime=runtime)

    assert root not in runtime.bootstrapped_signatures


def test_fixed_point_pass_env_override_is_fail_closed(monkeypatch) -> None:
    for raw in ("bad", "0", "-4"):
        monkeypatch.setenv("TEMPLATE_ACTIVE_INFERENCE_FIXED_POINT_PASSES", raw)
        assert gate_support._fixed_point_passes() == 1

    monkeypatch.setenv("TEMPLATE_ACTIVE_INFERENCE_FIXED_POINT_PASSES", "3")
    assert gate_support._fixed_point_passes() == 3
