...
\centering
\vspace*{2cm}
{\LARGE\bfseries Active Inference Multi-Track Exemplar\par}
\vspace{0.75em}
{\large Sheaf-Composed Manuscript with pymdp Sophisticated Inference\par}
\vfill
\makeatletter
{\@author\par}
\vspace{1em}
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio,alt={Sheaf track coverage matrix: 17 IMRAD rows \texorpdfstring{\ensuremath{\times}}{x} 34 fragment columns. Black = present (P), white = absent (---), gray = missing (M). Counts: 95 present / 95 bound / 0 missing. Generated from output/data/sheaf\_coverage\_matrix.json.}]{../figures/sheaf_coverage_heatmap.png}
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio,alt={Multi-track architecture: analytical, pymdp, and sheaf composition lanes mapped to 31 pipeline gates and 34 composable fragment types.}]{../figures/multi_track_architecture.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio,alt={Closed-form I(\textbackslash lambda) and an independent exact recomputation via total correlation for the symmetric Bernoulli-Ising toy across 21 grid points up to \textbackslash lambda\_\{\textbackslash max\} = 4; grid maximum 0.6031 nats. Both estimators are deterministic (no sampling), so the right panel is a cross-implementation agreement check (max residual 0 nats), not a sampling residual.}]{../figures/ising_mi_curve.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio,alt={GNN \textbackslash leftrightarrow ontology concordance for the Bernoulli--Ising toy (GNN v1.1).}]{../figures/gnn_ontology_concordance.png}
...
\begin{figure}
\centering
\includegraphics[width=0.85\linewidth,height=0.50\textheight,keepaspectratio,alt={T-maze generative model schematic (2-step policy horizon, state\_inference mode).}]{../figures/tmaze_schematic.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio,alt={Lean formalization boundary: module witnesses checked by lake build.}]{../figures/lean_boundary_status.png}
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio,alt={Semantic gluing graph: configured producers, generated evidence artifacts, and validation consumers for the multi-track sheaf certificate.}]{../figures/semantic_gluing_graph.png}
...
\begin{figure}
\centering
\includegraphics[width=0.98\linewidth,height=0.50\textheight,keepaspectratio,alt={Track-lane promotion map: 32 pipeline-to-sheaf rows with complete promotion status true. Left: seven promotion-rule obligations; right: sheaf fragment bindings.}]{../figures/track_lane_promotion_map.png}
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio,alt={Scholarship source map: 21 source rows across 21 method roles and 10 source families. Connected status: true; row evidence rederived: true.}]{../figures/scholarship_source_map.png}
...
\begin{figure}
\centering
\includegraphics[width=0.96\linewidth,height=0.50\textheight,keepaspectratio,alt={Security posture map: 9 controls, 7 enforced and 2 scoped as deferred; secret findings: 0; high-risk gaps: 0.}]{../figures/security_posture_map.png}
...
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio]{../figures/ising_mi_curve.png}
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio,alt={Closed-form Expected Free Energy decomposition over the finite T-maze policies. Left: G(\textbackslash pi) = risk + ambiguity (stacked), with the goal-seeking minimiser marked. Right: the pragmatic and epistemic values, which sum to -G(\textbackslash pi). Both forms are computed in closed form (no sampling) and satisfy risk + ambiguity + pragmatic + epistemic = 0 to machine precision.}]{../figures/efe_decomposition.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio,alt={Precision sweep over the closed-form T-maze policy posterior q(\textbackslash pi)\textbackslash propto\textbackslash exp(-\textbackslash gamma G) across 33 grid points up to \textbackslash gamma=16. Entropy falls from \textbackslash ln 4 to the \textbackslash ln|\textbackslash Pi\^{}\textbackslash star|{} floor (2 tied optima, second action irrelevant under the absorbing goal); the optimal-set mass crosses 0.99 at \textbackslash gamma=3. Entropy at \textbackslash gamma=1 is 1.1458 nats. Computed in closed form (no sampling).}]{../figures/precision_sweep.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio,alt={Belief entropy over time for the T-maze rollout (mean 0.3251 nats).}]{../figures/si_belief_entropy_curve.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio,alt={Observation and action traces for the T-maze rollout (action diversity 2).}]{../figures/si_obs_action_trace.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio,alt={Discrete action index over time for the pymdp T-maze rollout (policy length 2).}]{../figures/si_tmaze_actions.png}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio,alt={Invariant dashboard: 12 / 12 merged analytical and simulation checks from the validation registry.}]{../figures/invariant_dashboard.png}
...
\includegraphics[width=0.9\linewidth,height=0.50\textheight,keepaspectratio]{../figures/si_tmaze_actions.png}
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio,alt={Theorem traceability graph generated from 17 linked theorem rows and 207 proof-dependency edges.}]{../figures/theorem_traceability_graph.png}
...
\begin{figure}
\centering
\includegraphics[width=0.92\linewidth,height=0.50\textheight,keepaspectratio,alt={Causal-ablation heatmap: 36 source-backed rows joined to sensitivity and uncertainty artifacts; all effects source-backed: true.}]{../figures/causal_ablation_heatmap.png}
...
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio]{../figures/scholarship_source_map.png}
...
\includegraphics[width=0.98\linewidth,height=0.50\textheight,keepaspectratio]{../figures/track_lane_promotion_map.png}
...
\includegraphics[width=0.98\linewidth,height=0.50\textheight,keepaspectratio]{../figures/artifact_contract_map.png}
...
\includegraphics[width=0.96\linewidth,height=0.50\textheight,keepaspectratio]{../figures/security_posture_map.png}
...
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio]{../figures/sheaf_coverage_heatmap.png}
